 ...
 * @param name The name for the command based on its caller's context (used for performance
 *             measurements and debugging).
 * @param options Configuration options for the execution of Git.
 *
 * @return The stdout/stderr output. If the command exits with a code not in `successExitCodes`,
 *         or an error not in `expectedErrors`, a `GitError` will be thrown.
 */
export async function git(
  ...
  const result = await PerformanceMonitor.measure(commandName, () =>
...
function getDescriptionForError(error: DugiteError): string {
  switch (error) {
    ...
    case DugiteError.HTTPSAuthenticationFailed:
      return 'Authentication failed. You may not have permission to access the repository.';
